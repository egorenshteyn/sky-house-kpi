...
export default function LodgifySyncButton() {
  ...
  async function sync() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Lodgify sync failed");
      setResult(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Lodgify sync failed");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Pull reservations from Lodgify into bookings, calendar, dashboard, and CRM.
          </p>
        </div>
        <button
          type="button"
        ...
        >
          {loading ? "Syncing…" : "Sync reservations from Lodgify"}
        </button>
      </div>
